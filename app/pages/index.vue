...
import type { YooKassaPaymentRequest, YooKassaPaymentResponse } from '~/types/kassa'
import type { FormInstance, FormRules } from 'element-plus'

const postKassa = (value: number) => {
  return $fetch<YooKassaPaymentResponse>('/api/yookassa', {
    method: 'POST',
    body: {
      amount: {
        value: value,
...
const { execute, data, error, loading } = useMutation<YooKassaPaymentResponse, YooKassaPaymentRequest, any>(postKassa)
...
const url = computed(() => `${location.origin}/lk?email=${ruleForm.email}&name=${ruleForm.companyName}`)
const radio = ref(MAX_PRICE)




const rules = reactive<FormRules<typeof ruleForm>>({
  email: [{ trigger: 'blur', pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, }],
  companyName: [{ min: 2, max: 20, trigger: 'blur' }],
})
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      await execute(radio.value)

      if (data) {
        navigateTo(data.value?.confirmation.confirmation_url as string, { external: true })
      }
    }
  })
}
...
<template>
  <div>
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="141" height="24" viewBox="0 0 141 24" fill="none">
        ...
        <defs>
          <clipPath id="clip0_15_487">
            <rect width="140.722" height="23.0178" fill="white" transform="translate(0.138794 0.229492)" />
          </clipPath>
        </defs>
      </svg>
      <div class="block">
    ...
    <div class="item">
      <el-card class="card">
        <template #header>
          <el-text class="title" size="large">Оплата через Юкассу</el-text>
        </template>
        <el-form ref="ruleFormRef" style="max-width: 600px" :model="ruleForm" status-icon :rules="rules"
          label-width="auto" label-position="top" class="demo-ruleForm">
          <el-form-item size="large" label="Название компании" prop="companyName">
            <el-input v-model="ruleForm.companyName" type="text" autocomplete="off" />
          </el-form-item>
          <el-form-item size="large" label="Email" prop="email">
            <el-input v-model="ruleForm.email" type="email" autocomplete="off" />
          </el-form-item>
          <el-radio-group v-model="radio" size="large">
            <el-radio :value="MAX_PRICE" size="large">500 пользователей / {{ MAX_PRICE.toLocaleString('ru') }}
              рублей</el-radio>
            <el-radio :value="MIN_PRICE" size="large">100 пользователей / {{ MIN_PRICE.toLocaleString('ru') }}
              рублей</el-radio>
          </el-radio-group>
          <el-form-item>
            <el-button :disabled="loading" :loading="loading" class="button" size="large" type="primary"
        ...
        <transition mode="out-in">
          <el-alert v-if="error" class="alert" title="Что то пошло не так" type="error" />
        </transition>
      </el-card>
    </div>
  </div>
